...
module hbar_terminal_unit_holder() {
    difference() {
        ...
        union() {
            ...
            cube([unit_l+1, unit_w+1, unit_h - gear_max_d - cw_mount_wall*1], 
                center = true);
        }
    }
}
...
module hbar_belt_holder()
{
    difference() {
        union() {
            cube([belt_w + car_belt_border_w*2, car_belt_hldr_shell, car_main_l/2], 
                center = true);
            translate([0, car_belt_hldr_shell/2 + car_belt_border_h/2 - 
                belt_th/2 - 0.01, 0])
            cube([belt_w, car_belt_border_h, car_main_l/2], 
                center = true);
        }
        mirrorcp([1, 0, 0])
        translate([belt_w/2 + car_belt_border_w/2, 0, 0])
        rotate([90,0,0])
        cylinder(d = car_belt_mount_d, h = car_belt_hldr_shell*2, center = true);
    }
}
...
{
    difference() {
        square_channel(h = 20, w = 12, btm_th = 11, side_th = 1, l = 100);
        mirrorcp([0, 1, 0]) 
        translate([0, 10, -20])
        rotate([-3, 0, 0])
        cube([101, 10, 40], center = true);
    }
}
